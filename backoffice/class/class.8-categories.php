...
class c8_category {
	...
	protected $parent_id; /** @var int **/
	protected $lang_id; /** @var int **/
	protected $title_arr = []; /** @var array **/
	protected $description_arr = []; /** @var array **/
	protected $category_section; /** @var string **/
	protected $date; /** @var DateTime **/
	protected $date_update; /** @var DateTime **/
	protected $user_id; /** @var int **/
	protected $code; /** @var string **/
	protected $sort; /** @var int **/
	...
	/** @param int **/
	public function setParentId($pi) {$this->parent_id = $pi;}
	...
	/** @param string **/
	public function setCategorySection($s) {$this->category_section = $s;}

	/** @param boolean **/
	public function setPublished($p) {$this->published = $p;}
		...
		if ($trash->insert()) {
			if($db->query(sprintf(
				...
				if (count($category_rel) > 0 && is_array($category_rel)) {
					if($db->query(sprintf(
						"DELETE FROM %s_8_categories_rel WHERE category_id = %s", $cfg->db->prefix, $this->id
					))) {
						return TRUE;
					}
				}
	...
	public static function checkRel($id = null) {
		...
		if(!is_null($id) && $id != 0) {
			...
			$source = $db->query(sprintf(
				"SELECT * FROM %s_8_categories_rel WHERE category_id = '%s'", $cfg->db->prefix, $id
			));
	...
	public function returnCategoryRel() {
		...
		$source = $db->query(sprintf(
			"SELECT * FROM %s_8_categories_rel WHERE category_id = %s", $cfg->db->prefix, $this->id
		));
